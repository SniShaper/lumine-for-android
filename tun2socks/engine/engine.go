...
var (
	...
	// _customProxy holds a custom proxy used by mobile embedding.
	_customProxy proxy.Proxy
...
// StartErr starts the default engine and returns an error instead of exiting.
func StartErr() error {
	return start()
}
...
// StopErr shuts the default engine down and returns an error instead of exiting.
func StopErr() error {
	return stop()
}
...
// SetCustomProxy overrides proxy parsing and injects a custom proxy implementation.
func SetCustomProxy(p proxy.Proxy) {
	_engineMu.Lock()
	_customProxy = p
	_engineMu.Unlock()
}

// ClearCustomProxy removes the injected proxy override.
func ClearCustomProxy() {
	_engineMu.Lock()
	_customProxy = nil
	_engineMu.Unlock()
}
