...
- **`release.yml`**：手动触发并填写版本号（如 `v0.9.2`）。流程会把版本号以**字面量**形式写回 `android/app/build.gradle`（F-Droid 靠静态分析读取字面量）、提交该改动、在其上打 tag，随后签名构建四个 ABI 的 release APK，命名为 `lumine-<版本>-app-<abi>-release.apk` 并创建 GitHub Release。`versionCode` 默认取 `主版本*10000 + 次版本*100 + 修订号`，若不能大于上一版本号则拒绝发布。
