...
const (
	...
	// fakeIPTTLFloor is a fixed floor for how long a fake-IP<->domain
	// mapping is remembered, independent of any policy's DNSCacheTTL:
	// fake-IP TTL governs how long the tun-side synthetic mapping stays
	// resolvable, which is a different concern from upstream DNS cache
	// freshness.
	fakeIPTTLFloor = 60 * time.Second
)
...
func fakeIPLifetime(ttl uint32) time.Duration {
	...
	if fakeIPTTLFloor > lifetime {
		return fakeIPTTLFloor
	}
	return lifetime
}
