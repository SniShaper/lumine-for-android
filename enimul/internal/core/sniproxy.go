...
// SNIServe binds cmdAddr (or configAddr if unset) and serves SNI proxy
...
func SNIServe(cmdAddr, configAddr string, stop <-chan struct{}) {
	listenAddr := cmdAddr
	if listenAddr == "" {
	...
	for {
		...
		select {
		case <-stop:
			logger.Info("SNI proxy server stopped")
			return
		default:
		}
